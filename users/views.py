...
from .forms import RegisterUserForm, ProfileForm, UserForm, ChangePassForm, \
    PassResetRequest, SetPasswordForm
...
class UsersListView(braces_mixins.LoginRequiredMixin,
                    braces_mixins.StaffuserRequiredMixin, generic.ListView):
...
class SearchView(braces_mixins.LoginRequiredMixin,
                 braces_mixins.StaffuserRequiredMixin, generic.ListView):
    login_url = reverse_lazy("users:login")
    redirect_field_name = 'next'
    ...
    def get_queryset(self):
        ...
        users = User.objects.filter(
            Q(username__icontains=search) | Q(last_name__icontains=search) | Q(
                social_name__icontains=search) | Q(
                email__icontains=search)).distinct().order_by('social_name',
                                                              'username').exclude(
...
class CreateView(braces_mixins.LoginRequiredMixin,
                 braces_mixins.StaffuserRequiredMixin, LogMixin,
    ...
    def form_valid(self, form):
        ...
        super(CreateView, self).createLog(self.request.user, self.log_component,
...
class UpdateView(braces_mixins.LoginRequiredMixin,
                 braces_mixins.StaffuserRequiredMixin, LogMixin,
                 generic.UpdateView):
    log_component = 'user'
    log_action = 'update'
    ...
    def form_valid(self, form):
        ...
        msg = _('User "%s" updated successfully') % (
            self.object.get_short_name())
        ...
        super(UpdateView, self).createLog(self.request.user, self.log_component,
                                          self.log_action, self.log_resource,
                                          self.log_context)
...
class DeleteView(braces_mixins.LoginRequiredMixin, LogMixin,
                 generic.DeleteView):
    log_component = 'user'
    log_action = 'delete'
    log_resource = 'user'
    ...
    def delete(self, request, *args, **kwargs):
        ...
        super(DeleteView, self).createLog(self.request.user, self.log_component,
                                          self.log_action, self.log_resource,
                                          self.log_context)
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return self.response_class(request=self.request, template=template,
                                   context=context, using=self.template_engine,
                                   **response_kwargs)
...
class ForgotPassword(generic.FormView):
    ...
    success_message = _(
                    "Soon you'll receive an email with instructions to set your"
                    " new password. If you don't receive it in 24 hours, please"
                    " check your spam box.")
    error_message = _('No user is associated with this email address')
            ...
            if users.exists():
                for user in users:
                    ...
                    if mailsender.hostname == "example.com":
                        ...
                        mail_msg = EmailMessage(subject=subject, body=email,
                                                to=[
                                                    user.email],
                                                connection=backend)
    ...
    if request.POST:
        ...
        if user is not None:
            if not security.maintence or user.is_staff:
                ...
                notify_other_users(user, notification)
...
def notify_other_users(user, notification):
    users = User.objects.all().exclude(email=user.username)
